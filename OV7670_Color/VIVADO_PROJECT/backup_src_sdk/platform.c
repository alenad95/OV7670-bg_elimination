...
int init_platform(void)
{
	...
	//DDR Reference Reader
	result = init_ddr_to_axis_reader_ref();
	if(result != XST_SUCCESS)
	{
		xil_printf("There is an error about ddr_to_axis_reader_ref\n\r");
	}
	xil_printf("ddr_to_axis_reader_ref done \n\r");
	...
	//GPIO threshold
	result = init_axi_gpio_threshold();
	if(result != XST_SUCCESS)
	{
		xil_printf("There is an error about axi_gpio_threshold\n\r");
	}
	xil_printf("axi_gpio_threshold done \n\r");
